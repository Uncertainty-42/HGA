...
from utils.eval_utils_coco import AverageMeter, validate, MetricLogger
...
def _draw_chunk_heatmap_and_masks(
    ...
    with torch.no_grad():
        ...
        r_chunk = torch.zeros((81, H, W), device=s_masks[0].device)
        t_chunk = torch.zeros((81, H, W), device=s_masks[0].device)
        ...
        VISUALIZER.draw_heatmap(
            heatmap_data,
            tag=f"Logits81ch_Heatmap_Aggregated", # Corrected tag, representing aggregated result
            title=f"Sample: {sample_id} - Aggregated"
        )

        # 7. Generate predicted mask based on aggregated result (Argmax logic)
        # r_chunk shape is [81, H, W], take max index along the class dimension
...
def train(
    ...
    3. Result Aggregation:
       - Completeness guarantee: Only when the inner loop (num_chunks) has fully executed,
         ...
         instance masks onto an 81-channel canvas according to the category indices provided
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        for iter, data in enumerate(train_dataloader):
            try:
                ...
                batch_detections = []
                batch_voc_ids = []
                for sample_idx in range(images_weak.shape[0]):
                    # 1. Take the image from the batch
                    dino_input_tensor = images_weak[sample_idx]  # shape: [3, 1024, 1024]
                    
                    # 2. Transpose dimensions: (C, H, W) -> (H, W, C)
                    # The previously incorrect (1, 0, 2) caused rotation; must use (1, 2, 0)
                    ...
                    # class_label[0] shape is [81]. Indices: 0=bg, 1=aero, ... 20=tv
                    active_indices = torch.nonzero(class_label[sample_idx]).squeeze(1) 
                    ...
                    if target_class_names:
                        ...
                        if detections.class_id is not None:
                            ...
                            assert detections is not None, "[Error] In train_coco.py train: detections cannot be None!"
                        ...
                        if detections is not None and len(detections.xyxy) > 0: # type: ignore
                            prompts_sample = torch.tensor(detections.xyxy).to(device=fabric.device, dtype=torch.float64) # type: ignore
                            batch_prompts.append(prompts_sample)

                            # --- DINO input visualization probe ---
                            if sample_idx == 0 and VISUALIZER.is_active("dino_boxes"):
                                VISUALIZER.draw_detections(images_weak[0], detections, target_class_names, tag="Dino_Original", title="Original Image + DINO Boxes")
                            # ----------------------------------
                        else:
                            prompts_sample = torch.zeros((0, 4), device=fabric.device, dtype=torch.float64)
                            batch_prompts.append(prompts_sample)
                            
                        batch_detections.append(detections)
                        batch_voc_ids.append(local_to_voc_ids)
                    else:
                        batch_detections.append(None)
                        batch_voc_ids.append([])
                        batch_prompts.append(torch.zeros((0, 4), device=fabric.device, dtype=torch.float64))
                ...
                for sample_idx in range(images_weak.shape[0]):
                    if len(batch_prompts[sample_idx]) > 0:
                        sample_priors = {k: v[sample_idx:sample_idx+1] if isinstance(v, torch.Tensor) else v for k, v in priors_payload.items()}
                        # === [Logic Alignment] Process image-level background priors before the Chunk loop (B=1 logic) ===
                        model.prepare_for_image(sample_priors)
                        if ema_model is not None:
                            ema_model.prepare_for_image(sample_priors)
                        # =================================================================
                        all_prompts = batch_prompts[sample_idx].to(device=fabric.device, dtype=torch.float64)
                        ...
                        for chunk_idx in range(num_chunks):
                            ...
                            with GPU_MONITOR.scope(f"SAM_Chunk_{chunk_idx}_Forward"):
                                ...
                                with torch.no_grad():
                                    t_embeds, t_masks, t_iou, t_res = template_model(batched_input_weak[sample_idx:sample_idx+1], [chunk_prompts[0]], images_strong.shape[-2:])
                                ...
                                with torch.no_grad():
                                    s_embeds, s_masks, s_iou, s_res = u_teacher(batched_input_weak[sample_idx:sample_idx+1], [chunk_prompts[0]], images_strong.shape[-2:])

                                p_embeds, p_masks, p_iou, p_res = model(batched_input_strong[sample_idx:sample_idx+1], [chunk_prompts[0]], images_strong.shape[-2:])

                            # === Extract VOC category IDs for the current Chunk ===
                            if batch_detections[sample_idx] is not None:
                                assert batch_detections[sample_idx], "[Error] In train_coco.py train: batch_detections[sample_idx] cannot be None!"
                                chunk_detections = batch_detections[sample_idx][start:chunk_end]
                                assert isinstance(chunk_detections, sv.Detections), "[Error] In train_coco.py train: chunk_detections cannot be None!" # type: ignore
                                # print(chunk_detections.class_id, type(chunk_detections.class_id))
                                assert isinstance(chunk_detections.class_id, np.ndarray), "[Error] In train_coco.py train: chunk_detections cannot be None!"
                                curr_chunk_voc_ids = torch.tensor(
                                    [batch_voc_ids[sample_idx][cid] for cid in chunk_detections.class_id], 
                            ...
                            for i in range(len(p_masks)):
                                ...
                                with GPU_MONITOR.scope("Criterion_Evaluation"):
                                    loss_dict = criterion(
                                        ...
                                        output_hub = curr_hub,
                                        priors_payload=sample_priors,
                                        processed_priors=processed_priors_dict,
                                        chunk_voc_ids=curr_chunk_voc_ids,
                                        epoch=epoch, iter=iter, image_path=image_path
                                    )
                            ...
                            if sample_idx == 0:
                                ...
                                if VISUALIZER.is_active('baseline_monitoring'):
                                    _draw_chunk_heatmap_and_masks(s_masks, t_masks, batch_detections[sample_idx], start, chunk_end, image_path, gt_mask, VISUALIZER, batch_voc_ids[sample_idx], images[0])
                                ...
                                if VISUALIZER.is_active('top2_edge_alignment_monitoring'):
                                    ...
                                    if sample_priors:
                                        if 'namlab' in sample_priors:
                                            nam_raw = sample_priors['namlab'].float()
                                        ...
                                        if 'depth' in sample_priors:
                                            d_tensor = sample_priors['depth'].float()
            ...
            if iter > 0 and iter % 10400 == 0:
                iou: float = validate(fabric, cfg, args, ema_model, dino_model1, val_dataloader, 
                               cfg.name, epoch, train_iter=iter, save_dir=run_paths.log if run_paths else None,
                               fast_val=True)
                unwrapped_model = model.module if hasattr(model, "module") else model
                ...
                student_save_path = os.path.join(run_paths.ckpt, f"student_model_epoch_{epoch}_iter_{iter}_miou_{iou:.4f}_fast.pth")
                u_teacher_save_path = os.path.join(run_paths.ckpt, f"u_teacher_model_epoch_{epoch}_iter_{iter}_miou_{iou:.4f}_fast.pth")
        ...
        if epoch % cfg.eval_interval == 0:
            iou = validate(fabric, cfg, args, ema_model, dino_model1, val_dataloader, 
                           cfg.name, epoch, train_iter=None, save_dir=run_paths.log if run_paths else None,
                               fast_val=True)
            ...
            student_state = {"model": unwrapped_model, "optimizer": optimizer, "scheduler": scheduler}
            u_teacher_state = {"model": unwrapped_u_teacher_model}
            student_save_path = os.path.join(run_paths.ckpt, f"student_model_epoch_{epoch}_iter_{iter}_final_miou_{iou:.4f}_fast.pth")
            u_teacher_save_path = os.path.join(run_paths.ckpt, f"u_teacher_model_epoch_{epoch}_iter_{iter}_final_miou_{iou:.4f}_fast.pth")
...
def main(cfg: Box, args) -> None:
    ...
    if cfg.resume:
        if cfg.model.ckpt is None:
            raise ValueError("[in train_coco.py] When using resume, ckpt cannot be None!")
    ...
    unwrapped_u_teacher_model = ema_model.module if hasattr(ema_model, "module") else ema_model
    u_teacher_state = {"model": unwrapped_u_teacher_model}
    u_teacher_save_path = os.path.join(run_paths.ckpt, f"raw_model.pth")
    fabric.save(u_teacher_save_path, u_teacher_state)
    fabric.print("Saved raw model checkpoint, program exiting")
    sys.exit(0)
    ...
    if eval_before_training:
        ...
        # If resume is configured and the path exists, validate the U-teacher with loaded weights; otherwise validate the original T-Teacher
        if cfg.resume and cfg.model.ckpt is not None:
            eval_target = ema_model
            fabric.print(f"[Info] 🚀 Pre-training evaluation: Pre-trained weights detected, using model U-teacher to verify loading. Path: {cfg.model.ckpt}")
        else:
        ...
        validate(fabric, cfg, args, eval_target, dino_model1, val_data, name=cfg.name, epoch=0, save_dir=run_paths.log if run_paths else None, fast_val=True) # type: ignore
