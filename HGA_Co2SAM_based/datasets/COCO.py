...
class COCODataset(Dataset):
    def __init__(self, cfg, root_dir, transform=None, training=False, if_self_training=False):
        ...
        self.dataset_cfg = self.cfg.datasets.coco
        self.transform = transform
        self.if_self_training = if_self_training
        self.training = training

        # --- Core Definition ---
        # 0: background, 1: ... (matching the mapping logic in eval_utils)
        self.COCO_CLASSES = self.dataset_cfg.categories

        # 1. Directly scan the image directory
        ...
        # Determine subdirectory: train or val
        sub_dir = "train" if self.training else "val"
        # Image directory
        self.image_dir = os.path.join(self.root_dir, "JPEGImages", sub_dir)
        # Mask directory
        self.mask_dir = os.path.join(self.root_dir, "SegmentationClass", sub_dir)

        self.image_ids = sorted([
            f[:-4] for f in os.listdir(self.image_dir) if f.endswith('.jpg')
        ])
        ...
        if self.priors_enabled:
            self.priors_manager = PriorsManager(self.cfg, transform=None, dataset="coco", is_training=self.if_self_training)
        if training:
    ...
    def __getitem__(self, idx):
        ...
        mask_path = os.path.join(self.mask_dir, image_id.split("_")[-1] + '.png') # Original images have COCO_train2014_000000289140 prefix, strip it
        ...
        try:
            ...
            # Unify to 3-channel RGB, handle grayscale (L) and 4-channel (RGBA) images
            if image.ndim == 2:
        ...
        class_label = torch.zeros(len(self.COCO_CLASSES), dtype=torch.float)
        ...
        if self.training:
            ...
            if self.transform:
                ...
                # 3. Resize priors (use Resize's new is_label mode to ensure nearest-neighbor
