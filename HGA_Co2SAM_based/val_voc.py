...
from utils.eval_utils_voc import scores, sem_seg_postprocess, _draw_validation_visuals
...
import train_voc
sys.modules['__main__'].Co2SAMLRSchedule = train_voc.Co2SAMLRSchedule
...
def _infer_single(model, dino, cfg, args, image, gt_mask, image_path, class_label, priors_payload):
    ...
    # Filter out detection boxes where class_id is None (compatible with DINO occasional empty categories)
    if class_ids is not None:
        valid_mask = np.array([cid is not None for cid in class_ids], dtype=bool)
...
if __name__ == "__main__":
    ...
    if cfg.rand_seed is not None:
        train_voc.enforce_strict_determinism(cfg.rand_seed)
    ...
    with torch.no_grad():
        for idx, data in enumerate(tqdm(val_data, desc="Inference", ncols=100)):
            ...
            pred_file = (preds_dir if args.infer_set == "val" else test_result_dir) / f"{img_name}.png"
            if pred_file.exists():
